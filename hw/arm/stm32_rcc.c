...
static uint32_t stm32_RCC_CR_read(Stm32Rcc *s)
{
 ...
 * states.
 */
static void stm32_RCC_CR_write(Stm32Rcc *s, uint32_t new_value, bool init)
{
...
static uint32_t stm32_RCC_CFGR_read(Stm32Rcc *s)
{
...
static void stm32_RCC_CFGR_write(Stm32Rcc *s, uint32_t new_value, bool init)
{
    uint32_t new_PLLMUL, new_PLLXTPRE, new_PLLSRC;

    /* PLLMUL */
    new_PLLMUL = extract32(new_value, RCC_CFGR_PLLMUL_START, RCC_CFGR_PLLMUL_LENGTH);
    ...
    uint16_t prescaler;
    switch( s->RCC_CFGR_HPRE )
    {
    case  8: prescaler = 2; break;
    case  9: prescaler = 4; break;
    case 10: prescaler = 8; break;
    case 11: prescaler = 16; break;
    case 12: prescaler = 64; break;
    case 13: prescaler = 128; break;
    case 14: prescaler = 256; break;
    case 15: prescaler = 512; break;
    default: prescaler = 1; break;
    }
    clktree_set_scale( s->HCLK, 1, prescaler );

    /* PPRE2 */
    s->RCC_CFGR_PPRE2 = (new_value & RCC_CFGR_PPRE2_MASK) >> RCC_CFGR_PPRE2_START;

    switch( s->RCC_CFGR_PPRE2 )
    {
    case 4: prescaler = 2; break;
    case 5: prescaler = 4; break;
    case 6: prescaler = 8; break;
    case 7: prescaler = 16; break;
    default: prescaler = 1; break;
    }
    clktree_set_scale( s->PCLK2, 1, prescaler );

    /* PPRE1 */
    s->RCC_CFGR_PPRE1 = (new_value & RCC_CFGR_PPRE1_MASK) >> RCC_CFGR_PPRE1_START;

    switch( s->RCC_CFGR_PPRE1 )
    {
    case 4: prescaler = 2; break;
    case 5: prescaler = 4; break;
    case 6: prescaler = 8; break;
    ...
    clktree_set_scale( s->PCLK1, 1, prescaler );
    ...
    switch(s->RCC_CFGR_SW) {
        case 0x0:
        case 0x1:
        case 0x2: clktree_set_selected_input(s->SYSCLK, s->RCC_CFGR_SW); break;
        default:  hw_error("Invalid input selected for SYSCLK");         break;
    }
}

/* Write the APB2 peripheral clock enable register
 * Enables/Disables the peripheral clocks based on each bit. */
static void stm32_RCC_APB2ENR_write(Stm32Rcc *s, uint32_t new_value )
{
    stm32_rcc_periph_enable(s, new_value, STM32_ADC1,  RCC_APB2ENR_ADC1EN_BIT);
    stm32_rcc_periph_enable(s, new_value, STM32_ADC2,  RCC_APB2ENR_ADC2EN_BIT);
    stm32_rcc_periph_enable(s, new_value, STM32_UART1, RCC_APB2ENR_USART1EN_BIT);
    stm32_rcc_periph_enable(s, new_value, STM32_GPIOE, RCC_APB2ENR_IOPEEN_BIT);
    stm32_rcc_periph_enable(s, new_value, STM32_GPIOD, RCC_APB2ENR_IOPDEN_BIT);
    stm32_rcc_periph_enable(s, new_value, STM32_GPIOC, RCC_APB2ENR_IOPCEN_BIT);
    stm32_rcc_periph_enable(s, new_value, STM32_GPIOB, RCC_APB2ENR_IOPBEN_BIT);
    stm32_rcc_periph_enable(s, new_value, STM32_GPIOA, RCC_APB2ENR_IOPAEN_BIT);
    stm32_rcc_periph_enable(s, new_value, STM32_AFIO0, RCC_APB2ENR_AFIOEN_BIT);
    stm32_rcc_periph_enable(s, new_value, STM32_GPIOG, RCC_APB2ENR_IOPGEN_BIT);
    stm32_rcc_periph_enable(s, new_value, STM32_GPIOF, RCC_APB2ENR_IOPFEN_BIT);

    stm32_rcc_periph_enable(s, new_value, STM32_TIM1, RCC_APB2ENR_TIM1EN_BIT);
    stm32_rcc_periph_enable(s, new_value, STM32_TIM8, RCC_APB2ENR_TIM8EN_BIT);
 ...
 * Enables/Disables the peripheral clocks based on each bit. */
static void stm32_RCC_AHBENR_write( Stm32Rcc *s, uint32_t new_value, bool init )
{
    stm32_rcc_periph_enable( s, new_value, STM32_CRC , RCC_AHBENR_CRCEN_BIT );
    stm32_rcc_periph_enable( s, new_value, STM32_DMA1, RCC_AHBENR_DMA1EN_BIT);
 ...
 * Enables/Disables the peripheral clocks based on each bit. */
static void stm32_RCC_APB1ENR_write(Stm32Rcc *s, uint32_t new_value, bool init)
{
    stm32_rcc_periph_enable( s, new_value, STM32_TIM2, RCC_APB1ENR_TIM2EN_BIT);
    stm32_rcc_periph_enable( s, new_value, STM32_TIM3, RCC_APB1ENR_TIM3EN_BIT);
    stm32_rcc_periph_enable( s, new_value, STM32_TIM4, RCC_APB1ENR_TIM4EN_BIT);
    stm32_rcc_periph_enable( s, new_value, STM32_TIM5, RCC_APB1ENR_TIM5EN_BIT);
    stm32_rcc_periph_enable( s, new_value, STM32_TIM6, RCC_APB1ENR_TIM6EN_BIT);
    stm32_rcc_periph_enable( s, new_value, STM32_TIM7, RCC_APB1ENR_TIM7EN_BIT);

    stm32_rcc_periph_enable( s, new_value, STM32_UART5, RCC_APB1ENR_USART5EN_BIT);
    stm32_rcc_periph_enable( s, new_value, STM32_UART4, RCC_APB1ENR_USART4EN_BIT);
    stm32_rcc_periph_enable( s, new_value, STM32_UART3, RCC_APB1ENR_USART3EN_BIT);
    stm32_rcc_periph_enable( s, new_value, STM32_UART2, RCC_APB1ENR_USART2EN_BIT);

    //stm32_rcc_periph_enable( s, new_value, STM32_I2C1, RCC_APB1ENR_I2C1EN_BIT);
    //stm32_rcc_periph_enable( s, new_value, STM32_I2C2, RCC_APB1ENR_I2C2EN_BIT);

    s->RCC_APB1ENR = new_value & 0x00005e7d;
}

static uint32_t stm32_RCC_BDCR_read(Stm32Rcc *s)
{
...
static void stm32_RCC_BDCR_write(Stm32Rcc *s, uint32_t new_value, bool init)
{
    ...
    if(s->RTC_SEL){
        clktree_set_enabled(s->PERIPHCLK[STM32_RTC], (new_value >> RCC_BDCR_RTCEN_BIT)&0x01);
    }
}

/* Works the same way as stm32_RCC_CR_read */
static uint32_t stm32_RCC_CSR_read(Stm32Rcc *s)
{
...
/* Works the same way as stm32_RCC_CR_write */
static void stm32_RCC_CSR_write(Stm32Rcc *s, uint32_t new_value, bool init)
{
...
static uint64_t stm32_rcc_readw(void *opaque, hwaddr offset)
{
    ...
    switch (offset) {
        case RCC_CR_OFFSET:       return stm32_RCC_CR_read(s);
        case RCC_CFGR_OFFSET:     return stm32_RCC_CFGR_read(s);
        case RCC_CIR_OFFSET:      return 0;
        case RCC_APB2RSTR_OFFSET: STM32_NOT_IMPL_REG(offset, 4); return 0;
        case RCC_APB1RSTR_OFFSET: STM32_NOT_IMPL_REG(offset, 4); return 0;
        case RCC_AHBENR_OFFSET:   return s->RCC_AHBENR;
        case RCC_APB2ENR_OFFSET:  return s->RCC_APB2ENR;
        case RCC_APB1ENR_OFFSET:  return s->RCC_APB1ENR;
        case RCC_BDCR_OFFSET:     return stm32_RCC_BDCR_read(s);
        case RCC_CSR_OFFSET:      return stm32_RCC_CSR_read(s);
        case RCC_AHBRSTR:         STM32_NOT_IMPL_REG(offset, 4); return 0;
        case RCC_CFGR2_OFFSET:    STM32_NOT_IMPL_REG(offset, 4); return 0;
        default:                  STM32_BAD_REG(offset, 4); return 0;
    }
    return 0;
}
...
static void stm32_rcc_writew(void *opaque, hwaddr offset, uint64_t value)
{
    ...
    switch(offset) {
        case RCC_CR_OFFSET:       stm32_RCC_CR_write( s, value, false);      break;
        case RCC_CFGR_OFFSET:     stm32_RCC_CFGR_write( s, value, false);    break;
        case RCC_CIR_OFFSET:      /* Allow write but don't take any action*/ break;
        case RCC_APB2RSTR_OFFSET: STM32_NOT_IMPL_REG( offset, 4);            break;
        case RCC_APB1RSTR_OFFSET: STM32_NOT_IMPL_REG( offset, 4);            break;
        case RCC_AHBENR_OFFSET:   stm32_RCC_AHBENR_write( s, value, false);  break;
        case RCC_APB2ENR_OFFSET:  stm32_RCC_APB2ENR_write( s, value ); break;
        case RCC_APB1ENR_OFFSET:  stm32_RCC_APB1ENR_write( s, value, false); break;
        case RCC_BDCR_OFFSET:     stm32_RCC_BDCR_write( s, value, false);    break;
        case RCC_CSR_OFFSET:      stm32_RCC_CSR_write( s, value, false);     break;
        case RCC_AHBRSTR:         STM32_NOT_IMPL_REG( offset, 4);            break;
        case RCC_CFGR2_OFFSET:    STM32_NOT_IMPL_REG( offset, 4);            break;
        default:                  STM32_BAD_REG( offset, 4);                 break;
    }
}
...
{
    switch(size) {
        case 4:  return stm32_rcc_readw( opaque, offset );
        default: STM32_NOT_IMPL_REG( offset, size ); return 0;
    }
}
...
{
    switch(size) {
        case 4:  stm32_rcc_writew( opaque, offset, value); break;
        default: STM32_NOT_IMPL_REG( offset, size);        break;
    }
}
...
static void stm32_rcc_reset(DeviceState *dev)
{
    ...
    stm32_RCC_CR_write(     s, 0x00000083, true);
    stm32_RCC_CFGR_write(   s, 0, true);
    stm32_RCC_APB2ENR_write(s, 0 );
    stm32_RCC_APB1ENR_write(s, 0, true);
    stm32_RCC_AHBENR_write( s, 0, true);
    stm32_RCC_BDCR_write(   s, 0, true);
    stm32_RCC_CSR_write(    s, 0, true);
}
...
{
    Stm32Rcc *s = (Stm32Rcc *)opaque;
    uint32_t hclk_freq = clktree_get_output_freq(s->HCLK); //, ext_ref_freq;
    ...
    if( hclk_freq > 0 ) {
        ...
        updtCpuFreqHz( hclk_freq );

        /// hclk_freq *= 3; // FIXME simulation run slow after qemu updated.

        //double ps_instr = (double)hclk_freq/1000000;
        //ps_instr = 1000000/ps_instr;
        //m_arena->ps_per_inst = ps_instr;

        //printf("Qemu: stm32_rcc_hclk %i Hz %f ps\n", hclk_freq, m_arena->ps_per_inst );

        clock_set_hz( s->sysclk->source, hclk_freq );
        clock_propagate( s->sysclk->source );

        clktree_set_selected_input( s->PCLK1, 0 );
        clktree_set_selected_input( s->PCLK2, 0 );

        const uint64_t pclk1_freq = clktree_get_output_freq( s->PCLK1 );
        const uint64_t pclk2_freq = clktree_get_output_freq( s->PCLK2 );
        writeSimEvent(pclk2_freq, pclk1_freq, SIM_FREQ);

        printf("Qemu: APB1 freq: %f MHz\n", (double)pclk1_freq/1000000.0 );
        printf("Qemu: APB2 freq: %f MHz\n", (double)pclk2_freq/1000000.0 );
        //printf("Qemu:  %i %i %i\n", s->PCLK1->multiplier, s->PCLK1->divisor,s->PCLK1->input_freq );
        //printf("Qemu:  %i %i %i\n", s->PCLK2->multiplier, s->PCLK2->divisor,s->PCLK2->input_freq );

        //printf("Qemu:  %i %i\n", s->RCC_CFGR_PPRE1, s->RCC_CFGR_PPRE2 );
        fflush( stdout );
    }
}
...
void stm32_rcc_set_sysclk(Stm32Rcc* rcc, Clock *sysclk) /* DEVICE INITIALIZATION */
{
    rcc->sysclk = sysclk;
}

static void stm32_rcc_init_clk(Stm32Rcc *s) /* Set up the clock tree */
{
    int i;
    ...
    s->LSECLK = clktree_create_src_clk("LSE", s->osc32_freq, false);
     ...
     /* for RTCCLK */
    s->HSE_DIV128 = clktree_create_clk("HSE/128", 1, 128, true, CLKTREE_NO_MAX_FREQ, 0, s->HSECLK, NULL);
    ...
    s->PLLXTPRECLK = clktree_create_clk("PLLXTPRE", 1, 1, true, CLKTREE_NO_MAX_FREQ, CLKTREE_NO_INPUT, s->HSECLK, HSE_DIV2, NULL);
    /* PLLCLK contains both the switch and the multiplier, which are shown as
     * two separate components in the clock tree diagram.
     */
    s->PLLCLK = clktree_create_clk("PLLCLK", 0, 1, false, 72000000, CLKTREE_NO_INPUT, HSI_DIV2 , s->PLLXTPRECLK, NULL);
    s->SYSCLK = clktree_create_clk("SYSCLK", 1, 1,  true, 72000000, CLKTREE_NO_INPUT, s->HSICLK, s->HSECLK, s->PLLCLK, NULL);
    s->HCLK   = clktree_create_clk("HCLK"  , 0, 1,  true, 72000000,                0, s->SYSCLK , NULL);
    clktree_adduser(s->HCLK, hclk_upd_irq[0]);

    s->PCLK1 = clktree_create_clk("PCLK1", 0, 1, true, 36000000, 0, s->HCLK, NULL);
    s->PCLK2 = clktree_create_clk("PCLK2", 0, 1, true, 72000000, 0, s->HCLK, NULL);
    ...
    s->PERIPHCLK[STM32_AFIO0] = clktree_create_clk("AFIO" , 1, 1, false, CLKTREE_NO_MAX_FREQ, 0, s->PCLK2, NULL);
    ...
    s->PERIPHCLK[STM32_RTC]   = clktree_create_clk("RTC"  , 1, 1, false, CLKTREE_NO_MAX_FREQ,-1, s->LSECLK,s->LSICLK,s->HSE_DIV128, NULL);
