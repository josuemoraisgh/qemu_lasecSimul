...
static void stm32_rcc_hclk_upd_irq_handler(void *opaque, int n, int level)
{
    ...
    if( hclk_freq > 0 ) {
        ...
        waitForSynch();
        m_arena->regData = clktree_get_output_freq( s->PCLK1 );
        m_arena->regAddr = clktree_get_output_freq( s->PCLK2 );
        m_arena->simuAction = SIM_FREQ;
        m_arena->simuTime = getQemu_ps();

        printf("Qemu: APB1 freq: %f MHz\n", (double)m_arena->regData/1000000.0 );
        printf("Qemu: APB2 freq: %f MHz\n", (double)m_arena->regAddr/1000000.0 );
