...
void setInterrupt(void)
{
    ...
    DeviceState* intmatrix_dev = DEVICE( &esp32SOC->intmatrix );
    qemu_irq irq = qdev_get_gpio_in( intmatrix_dev, irqNumber);

    printf("Qemu: setInterrupt %i %i", irqNumber, irqLevel ); fflush( stdout );
...
uint32_t esp32_soc_get_apb_freq(void) { return esp32SOC->apb_clk_freq; }
uint32_t esp32_soc_get_cpu_freq(void) { return esp32SOC->cpu_clk_freq; }
...
static void esp32_clk_update( void* opaque, int n, int level )
{
    ...
    waitForSynch();
    m_arena->regData = s->cpu_clk_freq;
    m_arena->regAddr = s->apb_clk_freq;
