...
static const struct MemmapEntry {
    ...
    [ESP32_MEMREGION_FRAMEBUF]  = { 0x20000000, ESP_RGB_MAX_VRAM_SIZE } /* Virtual Framebuffer, used for the graphical interface */
};
...
{
    if (!m_arena || !esp32_soc) {
        return;
    }

    /*
     * irqNumber is the publication field: the Core writes irqLevel first and
     * irqNumber last.  Consuming it with an atomic exchange prevents the same
     * request from being injected twice when the run loop observes the arena
     * again before the Core publishes another interrupt.
     */
    uint64_t irq_number = qatomic_xchg(&m_arena->irqNumber, 0);
    uint64_t irq_level = qatomic_read(&m_arena->irqLevel);

    if (irq_number == 0 || irq_number >= ESP32_INT_MATRIX_INPUTS) {
        return;
    }

    qemu_set_irq(qdev_get_gpio_in(DEVICE(&esp32_soc->intmatrix), irq_number),
                 irq_level != 0);
}
...
static void esp32_soc_reset(DeviceState *dev)
{
    ...
    if( s->requested_reset & ESP32_SOC_RESET_PERIPH )
    {
        ...
        for (int i = 0; i < ESP32_UART_COUNT; ++i ) device_cold_reset( DEVICE(&s->uart[i]) );
        for (int i = 0; i < ESP32_FRC_COUNT;  ++i ) device_cold_reset( DEVICE(&s->frc_timer[i]) );
        for (int i = 0; i < ESP32_TIMG_COUNT; ++i ) device_cold_reset( DEVICE(&s->timg[i]) );

        s->timg[0].flash_boot_mode = flash_boot_mode;

        for (int i = 0; i < ESP32_SPI_COUNT; ++i ) device_cold_reset( DEVICE(&s->spi[i]));
        for (int i = 0; i < ESP32_I2C_COUNT; i++ ) device_cold_reset( DEVICE(&s->i2c[i]));

        device_cold_reset(DEVICE(&s->efuse));

        if ( s->eth     ) device_cold_reset( s->eth);
        if ( s->wifi_dev) device_cold_reset( s->wifi_dev);
...
{
    return esp32_soc ? esp32_soc->apb_clk_freq : 0;
}

uint32_t esp32_soc_get_cpu_freq(void)
{
    return esp32_soc ? esp32_soc->cpu_clk_freq : 0;
}

static void esp32_clk_update( void* opaque, int n, int level )
{
    //printf("Qemu: esp32_clk_update\n"); fflush( stdout );
    if( !level) return;
    ...
    writeSimEvent(s->apb_clk_freq, s->cpu_clk_freq, SIM_FREQ);

    //double ps_instr = (double)cpu_clk_freq/1000000;
    //ps_instr = 1000000/ps_instr;
    //if( m_arena->ps_per_inst != ps_instr )
    //{
    //    uint64_t qemuTime = getQemu_ps()/1000; // Update last cpu time and icount before ps_per_inst
    //    m_arena->ps_per_inst = ps_instr;
    //    printf("Qemu: esp32_clk_update CPU Freq Hz: %i %i %f at %lu\n", apb_clk_freq, cpu_clk_freq, ps_instr, qemuTime ); fflush( stdout );
    //}
}
...
static void esp32_soc_realize( DeviceState *dev, Error **errp )
{
    ...
    qdev_connect_gpio_out_named( DEVICE(&s->dport)   , ESP32_DPORT_APPCPU_RESET_GPIO, 0, qdev_get_gpio_in_named( dev, ESP32_RTC_CPU_RESET_GPIO, 1));
    qdev_connect_gpio_out_named( DEVICE(&s->dport)   , ESP32_DPORT_APPCPU_STALL_GPIO, 0, qdev_get_gpio_in_named( dev, ESP32_RTC_CPU_STALL_GPIO, 1));
    qdev_connect_gpio_out_named( DEVICE(&s->rtc_cntl), ESP32_DPORT_CLK_UPDATE_GPIO  , 0, qdev_get_gpio_in_named( dev, ESP32_RTC_CLK_UPDATE_GPIO, 0));
    ...
    qdev_connect_gpio_out_named( DEVICE(&s->rtc_cntl), ESP32_RTC_DIG_RESET_GPIO , 0, qdev_get_gpio_in_named( dev, ESP32_RTC_DIG_RESET_GPIO, 0));
    qdev_connect_gpio_out_named( DEVICE(&s->rtc_cntl), ESP32_RTC_CLK_UPDATE_GPIO, 0, qdev_get_gpio_in_named( dev, ESP32_RTC_CLK_UPDATE_GPIO, 0));

    for (int i = 0; i < ms->smp.cpus; ++i)
    {
        qdev_connect_gpio_out_named( DEVICE(&s->rtc_cntl), ESP32_RTC_CPU_RESET_GPIO, i, qdev_get_gpio_in_named( dev, ESP32_RTC_CPU_RESET_GPIO, i));
        qdev_connect_gpio_out_named( DEVICE(&s->rtc_cntl), ESP32_RTC_CPU_STALL_GPIO, i, qdev_get_gpio_in_named( dev, ESP32_RTC_CPU_STALL_GPIO, i));
    }
    ...
    for (int i = 0; i < ESP32_FRC_COUNT; ++i)
    {
        ...
        sysbus_connect_irq( SYS_BUS_DEVICE(&s->frc_timer[i]), 0, qdev_get_gpio_in(intmatrix_dev, ETS_TIMER1_INTR_SOURCE + i));
    }
    ...
    for (int i = 0; i < ESP32_TIMG_COUNT; ++i)
    {
        ...
        qdev_connect_gpio_out_named( DEVICE(&s->timg[i]), ESP32_TIMG_WDT_CPU_RESET_GPIO, 0, qdev_get_gpio_in_named(dev, ESP32_TIMG_WDT_CPU_RESET_GPIO, i));
        qdev_connect_gpio_out_named( DEVICE(&s->timg[i]), ESP32_TIMG_WDT_SYS_RESET_GPIO, 0, qdev_get_gpio_in_named(dev, ESP32_TIMG_WDT_SYS_RESET_GPIO, i));
    }
    ...
    for (int i = 0; i < ESP32_SPI_COUNT; ++i)
    {
        ...
        sysbus_connect_irq( SYS_BUS_DEVICE(&s->spi[i]), 0, qdev_get_gpio_in(intmatrix_dev, ETS_SPI0_INTR_SOURCE + i));
        s->spi[i].number = i;
    }

    qdev_realize( DEVICE(&s->rmt), &s->periph_bus, &error_fatal);
    esp32_soc_add_periph_device( sys_mem, &s->rmt, DR_REG_RMT_BASE);
    sysbus_connect_irq( SYS_BUS_DEVICE(&s->rmt), 0, qdev_get_gpio_in(intmatrix_dev, ETS_RMT_INTR_SOURCE));
    ...
    sysbus_connect_irq( SYS_BUS_DEVICE(&s->efuse), 0, qdev_get_gpio_in(intmatrix_dev, ETS_EFUSE_INTR_SOURCE));
    ...
    qdev_connect_gpio_out_named( DEVICE(&s->efuse), ESP32_EFUSE_UPDATE_GPIO, 0, qdev_get_gpio_in_named( DEVICE(&s->flash_enc), ESP32_FLASH_ENCRYPTION_EFUSE_UPDATE_GPIO, 0));
    qdev_connect_gpio_out_named( DEVICE(&s->dport), ESP32_DPORT_FLASH_ENC_EN_GPIO, 0, qdev_get_gpio_in_named( DEVICE(&s->flash_enc), ESP32_FLASH_ENCRYPTION_ENC_EN_GPIO, 0));
    qdev_connect_gpio_out_named( DEVICE(&s->dport), ESP32_DPORT_FLASH_DEC_EN_GPIO, 0, qdev_get_gpio_in_named( DEVICE(&s->flash_enc), ESP32_FLASH_ENCRYPTION_DEC_EN_GPIO, 0));
...
#define TYPE_ESP32_SIMUL_MACHINE MACHINE_TYPE_NAME("esp32-simul")
...
{
    /*
     * Keep the historical stand-alone ESP32 machine's TMP105 fixture.  The
     * esp32-simul alias intentionally skips it because its external I2C bus is
     * driven by the LasecSimul electrical Core.
     */
...
{
    const bool simul_mode =
        object_dynamic_cast(OBJECT(machine), TYPE_ESP32_SIMUL_MACHINE) != NULL;
    ...
    if( load_elf_filename )
    {
        ...
        printf("Qemu: loading ROM binary '%s'\n", rom_binary );
        ...
        if( size < 0) {
            error_report("Error: could not load ROM binary '%s'\n", rom_binary);
            exit(1);
        }
        g_free(rom_binary);
        ...
        printf("Qemu: loading APP binary '%s'\n", rom_binary );
...
static void esp32_simul_machine_class_init(ObjectClass *oc, void *data)
{
    MachineClass *mc = MACHINE_CLASS(oc);

    mc->desc = "Espressif ESP32 machine with LasecSimul bridge";
    mc->is_default = false;
}
...
static const TypeInfo esp32_simul_info = {
    .name = TYPE_ESP32_SIMUL_MACHINE,
    .parent = TYPE_ESP32_MACHINE,
    .class_init = esp32_simul_machine_class_init,
};

static void esp32_machine_type_init(void)
{
    type_register_static(&esp32_info);
    type_register_static(&esp32_simul_info);
}
