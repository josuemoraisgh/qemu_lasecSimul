...
{
    Esp32LEDCState *s = ESP32_LEDC(opaque);
    const char *trace = getenv("LASECSIMUL_TRACE_ANALOG");
    if (trace && trace[0] && strcmp(trace, "0") != 0) {
        printf("[LasecSimul][LEDC] write addr=0x%03llx value=0x%08llx\n",
    ...
    case A_LEDC_LSCH7_DUTY_REG:{
        ...
        /* O adaptador externo precisa do DUTY_REG bruto para reconstruir a forma de onda no pino.
         * `intensity` continua calculado para o estado interno/LED legado, mas nao substitui mais o
         * valor MMIO publicado na arena. */
        (void)intensity;
        }break;
    }
