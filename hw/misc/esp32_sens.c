...
static bool esp32_analog_trace_enabled(void)
{
    const char *value = getenv("LASECSIMUL_TRACE_ANALOG");
...
static uint64_t esp32_sens_read(void *opaque, hwaddr addr, unsigned int size)
{
    ...
    case 0x54://SENS_MEAS1_START_SAR
        r = readReg(s->iomem.addr + addr);
        if (esp32_analog_trace_enabled()) {
            printf("[LasecSimul][ADC] read unit=1 channel=%d raw=%u\n", s->channel1, r);
            fflush(stdout);
        }
        return 0x10000 + r;
	...
	r = readReg(s->iomem.addr + addr);
        if (esp32_analog_trace_enabled()) {
            printf("[LasecSimul][ADC] read unit=2 channel=%d raw=%u\n", s->channel2, r);
            fflush(stdout);
        }
	return 0x10000 + r;
    }
